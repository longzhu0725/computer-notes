...
特性： 异步性：与 **CPU当前执行指令无关** ， **随时可能发生** ；可屏蔽：可以 **通过中断屏蔽位暂时禁止某些中断** ；可嵌套： **高级中断可以打断低级中断处理**

分类： **硬件中断** （Hardware Interrupt） 可屏蔽中断（INTR）：可 **通过CLI指令屏蔽 非屏蔽中断** （NMI）： **不可屏蔽** ，如内存奇偶校验错
...
特性： 同步性： **特定指令** 执行必然产生；不可屏蔽： **无法通过软件屏蔽** ；精确性：可以精确定位到出错指令
...
![image](/assets/interrupts-exceptions.png)
...
← [进程调度算法有哪些？适用场景是什么？](https://notes.kamacoder.com/base/scheduling-algorithms-scenarios.html) [什么是中断和异常，它们有什么区别？](https://notes.kamacoder.com/base/interrupts-vs-exceptions.html) →


## 与其他知识的关联

- [中断和异常的区别](/computer-basics/os/中断和异常的区别.html) — 中断与异常对比
- [中断处理流程](/computer-basics/os/中断处理流程.html) — 中断的处理过程
- [用户态和内核态](/computer-basics/os/用户态和内核态.html) — 中断与状态切换

## 来源与延伸阅读

- raw/2026-06-27_interrupts-exceptions.md — 卡码笔记原文
