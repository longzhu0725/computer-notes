...
1. **错误日志 (Error Log)：**
	- 错误日志是 MySQL 服务器 **最重要** 的诊断日志，记录了服务器生命周期中的关键事件，包括 服务器的启动和关闭过程、运行期间发生的严重错误（如崩溃）、警告信息（如配置问题）以及一些通知信息 。
...
1. **MySQL 日志文件在架构中的位置与作用** ，示意图如下：  
	![image](/assets/MySQL_log_files_sorts.jpeg)
2. **面试官可能的追问1：Redo Log 和 Binlog 有什么区别？**
...
3. **面试官可能的追问2：Undo Log 在 MVCC 中是如何工作的？**
...
- [事务的四大特性ACID](/computer-basics/database/事务的四大特性acid.html) — redo log保证持久性
- [MVCC机制](/computer-basics/database/mvcc机制.html) — undo log版本链
- [MySQL存储引擎](/computer-basics/database/mysql存储引擎.html) — 日志与存储引擎
- [Redis持久化RDB与AOF](/computer-basics/database/redis持久化rdb与aof.html) — 持久化类比

## 来源与延伸阅读

- raw/2026-06-27_mysql-logs.md — 卡码笔记原文
