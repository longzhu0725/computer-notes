...
2. **缓存击穿 / Cache Stampede** ：某个 **热点 key** 在高并发场景下刚好过期，大量请求在同一时刻发现缓存失效，于是一起回源数据库。
...
2. 三类问题示意 ![image](/assets/20260407_Redis_Penetration_Stampeding_Avalanche.jpeg)
...
- [Redis布隆过滤器](/computer-basics/database/redis布隆过滤器.html) — 解决缓存穿透
- [Redis缓存淘汰策略](/computer-basics/database/redis缓存淘汰策略.html) — 缓存淘汰与雪崩
- [Redis分布式锁](/computer-basics/database/redis分布式锁.html) — 解决缓存击穿
- [Redis过期策略](/computer-basics/database/redis过期策略.html) — 过期导致雪崩

## 来源与延伸阅读

- raw/2026-06-27_redis-cache-problems.md — 卡码笔记原文
