...
2. **一致性（Consistency）** ：
	- 一致性是指事务必须使数据库从 **一个一致性状态** 转换到 **另一个一致性状态** 。
		...
		- 事务执行前数据库处于一致性状态，事务执行完毕后，无论成功提交还是失败回滚，数据库都必须 **仍然处于** 一致性状态。 一致性 是事务的 **目标** ，而 原子性、隔离性和持久性 是实现一致性的 **手段** 。
3. **隔离性（Isolation）：**
	- 隔离性是指多个 **并发执行** 的事务之间相互 **隔离** ，互不干扰。
		- 每个事务都应该 **感觉** 自己是系统中唯一正在运行的事务，即使有其他事务同时在执行，一个事务的中间结果对其他事务也是不可见的。
		- 隔离性是用来解决 **并发事务** 所带来的问题，如 脏读、不可重复读和幻读 。数据库通过不同的 **事务隔离级别** 来控制并发事务之间的隔离程度，从而在数据一致性和并发性能之间进行权衡。隔离级别越高，隔离性越好，但并发性能可能越低。
4. **持久性（Durability）：**
...
1. **事务的 原子性(A) 示意图如下** ：  
	![image](/assets/transaction_Atomicity.jpeg)
2. **事务的 隔离性(I) 示意图如下** ：  
	![image](/assets/transaction_isolation.jpeg)
3. **事务的 持久性(D) 示意图如下** ：  
	![image](/assets/transaction_durability.jpeg)
...
- [事务隔离级别](/computer-basics/database/事务隔离级别.html) — 隔离性的具体实现
- [MVCC机制](/computer-basics/database/mvcc机制.html) — 并发版本控制
- [数据库锁类型](/computer-basics/database/数据库锁类型.html) — 锁保证隔离性
- [MySQL日志文件](/computer-basics/database/mysql日志文件.html) — 日志保证持久性

## 来源与延伸阅读

- raw/2026-06-27_acid-properties.md — 卡码笔记原文
