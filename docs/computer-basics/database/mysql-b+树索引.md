...
1. **MySQL InnoDB的 B+树索引（主键，聚簇索引）** ， 示意图如下：  
	![image](/assets/index_B+Tree_PRIMARYKEY.jpeg)
2. **MySQL Innodb的 B+树索引（二级索引，非聚簇）** ，示意图如下：  
	![image](/assets/index_sorts_B+Tree_innoDB.jpeg)
3. **InnoDB 存储引擎在页面 (Page) 级别实现的 B+ 树索引结构** ，示意图如下：  
	![image](/assets/index_B+Tree_pageLevel.jpeg)
...
- [索引的种类](/computer-basics/database/索引的种类.html) — 索引分类总览
- [MySQL存储引擎](/computer-basics/database/mysql存储引擎.html) — InnoDB与MyISAM索引差异
- [索引失效场景](/computer-basics/database/索引失效场景.html) — B+树索引使用限制
- [创建索引的时机](/computer-basics/database/创建索引的时机.html) — 何时建B+树索引

## 来源与延伸阅读

- raw/2026-06-27_b-plus-tree-index.md — 卡码笔记原文
