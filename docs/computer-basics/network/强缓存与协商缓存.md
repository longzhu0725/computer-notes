...
1. **强缓存 和 协商缓存 的工作流程** ，如下图所示：  
	![image](/assets/two_cachings.jpeg)
...
4. **为什么需要ETag？**
	- **解决Last-Modified的不足** ： 例如，当文件在1秒内被多次修改，或文件被修改后又被还原（内容未变但时间戳变化）， **Last-Modified** 会错误地认为资源已更新，而 **ETag** 能准确判断内容是否变化。
		- **动态内容验证** ： 适用于动态生成的内容（如API响应），无法通过修改时间判断是否更新。

## 与其他知识的关联

- [HTTP请求头部字段](/computer-basics/network/http请求头部字段.html) — HTTP Headers详解
- [HTTP常见状态码](/computer-basics/network/http常见状态码.html) — HTTP响应状态码
- [HTTP 1.0与1.1的区别](/computer-basics/network/http-10与11的区别.html) — HTTP版本演进

## 来源与延伸阅读

- raw/2026-06-27_cache-control.md — 卡码笔记原文
