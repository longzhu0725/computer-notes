...
- [STL 容器选型](./stl-容器选型.md) - 选型决策
- [vector 底层原理和扩容机制](./vector-底层原理和扩容机制.md) - allocator 在 vector 扩容时的角色
- [map vs unordered_map](./map-vs-unordered_map.md) - 节点式容器的内存管理
- [placement new](./placement-new.md) - allocator.construct 的实现机制
- [内存碎片](./内存碎片.md) - 大量分配释放导致的问题
- [new vs malloc](./new-vs-malloc.md) - 与底层分配的关系
