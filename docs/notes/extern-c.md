...
- [cpp/多态](/notes/多态.html)：C++ 的虚函数依赖 vtable（一种 RTTI 数据结构），C 调用约定无法处理 vtable 查找——所以 **extern "C" 不能修饰虚函数**
- [cpp/继承](/notes/继承.html)：vtable 是 C++ 类的"运行时类型信息"，跨语言接口时 C 无法看到这些，所以**所有 C 接口只能是自由函数 + void* 句柄**
- [cpp/指针 vs 引用](/notes/指针-vs-引用.html)：extern "C" 接口通常用 `void*` 或 `intptr_t` 句柄传引用，而不是真正的 C++ 引用（C 无法直接处理）
- [cpp/struct vs class](/notes/struct-vs-class.html)：与 C 互操作的 struct 必须保持 C 风格（无成员函数、无继承、无访问控制）
- [cpp/类型转换](/notes/类型转换.html)：`static_cast` 在跨 extern "C" 边界时是必要的（`void*` ↔ `T*`）
- [cpp/static vs const](/notes/static-vs-const.html)：`static const` 全局变量在跨翻译单元访问时也需要 `extern "C"` 保持符号一致

## 来源与延伸阅读

- raw/2026-06-22_extern-c — 原始资料（卡码笔记 C++ 面试题系列，2026-05-23）
