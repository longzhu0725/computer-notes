...
- [cpp/inline vs 宏](/notes/inline-vs-宏.html)：C++11 起 `auto` 替代了大量"宏函数"（如 `FOREACH`）
- [cpp/类型转换](/notes/类型转换.html)：`decltype` 配合 `std::declval` 是 SFINAE 的核心
- [cpp/static vs const](/notes/static-vs-const.html)：`auto` 默认会丢失顶层 const，需要 `const auto&`
- [cpp/struct vs class](/notes/struct-vs-class.html)：模板元编程中常用 `decltype(std::declval<T>().member())` 检查 struct/class 是否有某成员
- [cpp/继承](/notes/继承.html)：返回类型后置（`auto f() -> decltype(...)`）是模板方法模式中常用
- [cpp/多态](/notes/多态.html)：智能指针 + auto 是现代 C++ 多态代码的标准模式（`auto p = std::make_unique<Derived>();`）
- [cpp/volatile](/notes/volatile.html)：auto 推导时不会添加 volatile 限定符，需要显式 `volatile auto&`

## 来源与延伸阅读

- raw/2026-06-22_auto-vs-decltype — 原始资料（卡码笔记 C++ 面试题系列，2026-05-23）
